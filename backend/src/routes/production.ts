...
router.get('/:idOrDate', async (req, res, next) => {
  try {
    const param = req.params.idOrDate;
    const isDate = /^\d{4}-\d{2}-\d{2}$/.test(param);
    const data = isDate
      ? await productionService.getProductionByDate(param, req.farmId)
      : await productionService.getProductionById(param, req.farmId);
    res.json({ data });
  } catch (error) {
    next(error);
  }
